...
import { financeApi, formatCurrency, StudentFeeDetails } from "@/src/config/financeApi";
...
export default function StudentFeeDetailScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        Alert.alert("خطا", "اطلاعات شهریه یافت نشد");
        router.back();
      }
    } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "PENDING":
        return { text: "در انتظار", color: Colors.info, bg: `${Colors.info}15`, icon: "hourglass" };
      case "OVERDUE":
        return { text: "سررسید گذشته", color: Colors.danger, bg: `${Colors.danger}15`, icon: "alert-circle" };
      default:
      ...
      >
        ...
        <View style={styles.summaryCard}>
          <View style={styles.summaryHeader}>
            <View style={styles.avatarContainer}>
              <Text style={styles.avatarText}>{data.studentName.charAt(0)}</Text>
            </View>
        ...
        {data.fees.length === 0 ? (
          ...
          data.fees.map((fee) => {
            ...
            const billingPeriod = getBillingPeriod(fee.billingMonth, fee.billingYear);
            const isPending = fee.status !== "PAID" && fee.status !== "CANCELLED";
            const paymentPercent = fee.amount > 0 ? (fee.paidAmount / fee.amount) * 100 : 0;
            ...
            return (
              <View key={fee.id} style={styles.feeCard}>
                ...
                {fee.discount && (
                  <View style={styles.discountRow}>
                    ...
                    <Text style={styles.discountText}>
                      ...
                      {fee.discount.type === "PERCENTAGE" ? `${fee.discount.value}%` : formatCurrency(fee.discount.value)})
                    </Text>
                  </View>
                )}
                ...
                <View style={styles.feeBody}>
                  <View style={styles.feeRow}>
                    ...
                    <View style={styles.feeItem}>
                      ...
                      <Text style={[styles.feeValue, { color: fee.remainingAmount > 0 ? Colors.danger : Colors.success }]}>
                      ...
                      <View
                        style={[
                          styles.progressFill,
                          {
                            ...
                            backgroundColor: fee.status === "PAID" ? Colors.success : Colors.primary,
                          },
                        ]}
                      />
                    </View>
                ...
                {isPending && fee.remainingAmount > 0 && (
                  <TouchableOpacity style={styles.payButton} onPress={() => openPayModal(fee)} activeOpacity={0.7}>
                ...
                {fee.payments && fee.payments.length > 0 && (
                  <View style={styles.paymentHistory}>
                    <Text style={styles.paymentHistoryTitle}>سابقه پرداخت‌ها</Text>
                    {fee.payments.map((payment) => (
                      <View key={payment.id} style={styles.paymentItem}>
                        ...
                        <View style={styles.paymentItemRight}>
                          ...
                          <Text style={styles.paymentBy}>{payment.confirmedBy}</Text>
                        </View>
                      </View>
                    ))}
                  </View>
          ...
          <View style={styles.modalContent}>
              ...
              {selectedFee && (
                <View style={styles.paymentInfoCard}>
                  <View style={styles.paymentInfoRow}>
                    ...
                    <Text style={styles.paymentInfoValue}>{selectedFee.title}</Text>
                  </View>
                  <View style={styles.paymentInfoRow}>
                    <Text style={styles.paymentInfoLabel}>مبلغ کل:</Text>
                    <Text style={styles.paymentInfoValue}>{formatCurrency(selectedFee.amount)}</Text>
                  </View>
                  <View style={styles.paymentInfoRow}>
                    <Text style={styles.paymentInfoLabel}>مانده:</Text>
                    <Text style={[styles.paymentInfoValue, { color: Colors.danger }]}>
                ...
                <View style={styles.methodRow}>
                  {[
                    ...
                    <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                    >
                      <Text style={[styles.methodChipText, paymentForm.paymentMethod === m.value && styles.methodChipTextActive]}>
                        {m.label}
                      </Text>
                    </TouchableOpacity>
                  ))}
            ...
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                {submitting ? <ActivityIndicator size="small" color="white" /> : <Text style={styles.modalApplyText}>ثبت پرداخت</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  summaryCard: { backgroundColor: Colors.card, borderRadius: 16, padding: 16, marginBottom: 20, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 3 },
  ...
  summaryInfo: { flex: 1 },
  studentName: { fontSize: 18, fontWeight: "bold", color: Colors.text, fontFamily: "Vazirmatn", marginBottom: 2 },
  className: { fontSize: 13, color: Colors.textSecondary, fontFamily: "Vazirmatn" },
  
  summaryStats: { flexDirection: "row", paddingVertical: 12, borderTopWidth: 1, borderTopColor: Colors.border, borderBottomWidth: 1, borderBottomColor: Colors.border, marginBottom: 12 },
  statItem: { flex: 1, alignItems: "center" },
  statDivider: { width: 1, backgroundColor: Colors.border },
  statValue: { fontSize: 16, fontWeight: "bold", fontFamily: "Vazirmatn", marginBottom: 2 },
  ...
  sectionTitle: { fontSize: 17, fontWeight: "bold", color: Colors.text, fontFamily: "Vazirmatn", marginBottom: 14, marginTop: 8 },
  
  feeCard: { backgroundColor: Colors.card, borderRadius: 14, padding: 14, marginBottom: 12, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3, elevation: 2 },
  ...
  feeTitle: { fontSize: 14, fontWeight: "600", color: Colors.text, fontFamily: "Vazirmatn", flex: 1 },
  periodBadge: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6, backgroundColor: `${Colors.primary}10` },
  ...
  discountRow: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8, paddingHorizontal: 8, paddingVertical: 4, backgroundColor: `${Colors.success}10`, borderRadius: 6, alignSelf: "flex-start" },
  discountText: { fontSize: 11, color: Colors.success, fontFamily: "Vazirmatn" },
  
  feeBody: { backgroundColor: Colors.background, borderRadius: 10, padding: 12, marginBottom: 10, gap: 8 },
  feeRow: { flexDirection: "row", gap: 8 },
  feeItem: { flex: 1, alignItems: "center" },
  feeLabel: { fontSize: 10, color: Colors.textSecondary, fontFamily: "Vazirmatn", marginBottom: 2 },
  feeValue: { fontSize: 13, fontWeight: "500", color: Colors.text, fontFamily: "Vazirmatn" },
  
  progressSection: { flexDirection: "row", alignItems: "center", gap: 8 },
  progressBar: { flex: 1, height: 6, backgroundColor: `${Colors.textSecondary}20`, borderRadius: 3, overflow: "hidden" },
  progressFill: { height: "100%", borderRadius: 3 },
  progressText: { fontSize: 10, color: Colors.textSecondary, fontFamily: "Vazirmatn" },
  
  feeFooter: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 10 },
  feeDate: { fontSize: 11, color: Colors.textSecondary, fontFamily: "Vazirmatn" },
  ...
  paymentHistory: { marginTop: 12, paddingTop: 10, borderTopWidth: 1, borderTopColor: Colors.border },
  paymentHistoryTitle: { fontSize: 12, fontWeight: "600", color: Colors.textSecondary, fontFamily: "Vazirmatn", marginBottom: 8, textAlign: "right" },
  paymentItem: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", backgroundColor: `${Colors.primary}05`, borderRadius: 8, padding: 8, marginBottom: 6 },
  paymentItemLeft: { alignItems: "flex-start" },
  paymentAmount: { fontSize: 13, fontWeight: "bold", color: Colors.success, fontFamily: "Vazirmatn" },
  paymentMethod: { fontSize: 10, color: Colors.textSecondary, fontFamily: "Vazirmatn", marginTop: 2 },
  paymentItemRight: { alignItems: "flex-end" },
  paymentDate: { fontSize: 11, color: Colors.textSecondary, fontFamily: "Vazirmatn" },
  paymentBy: { fontSize: 10, color: Colors.textSecondary, fontFamily: "Vazirmatn", marginTop: 2 },
  
  emptyState: { alignItems: "center", paddingVertical: 40, backgroundColor: Colors.card, borderRadius: 12 },
  ...
  modalCancelBtn: { flex: 1, paddingVertical: 14, borderRadius: 10, backgroundColor: Colors.background, alignItems: "center", borderWidth: 1, borderColor: Colors.border },
  modalCancelText: { fontSize: 15, fontWeight: "500", color: Colors.textSecondary, fontFamily: "Vazirmatn" },
  modalApplyBtn: { flex: 2, paddingVertical: 14, borderRadius: 10, backgroundColor: Colors.primary, alignItems: "center" },
  modalApplyText: { fontSize: 15, fontWeight: "600", color: "white", fontFamily: "Vazirmatn" },
  
  paymentInfoCard: { backgroundColor: Colors.background, borderRadius: 10, padding: 14, marginBottom: 16 },
  paymentInfoRow: { flexDirection: "row", justifyContent: "space-between", marginBottom: 8 },
  paymentInfoLabel: { fontSize: 13, color: Colors.textSecondary, fontFamily: "Vazirmatn" },
  paymentInfoValue: { fontSize: 13, fontWeight: "500", color: Colors.text, fontFamily: "Vazirmatn" },
  
  formGroup: { marginBottom: 16 },
  formLabel: { fontSize: 13, fontWeight: "500", color: Colors.text, fontFamily: "Vazirmatn", marginBottom: 6, textAlign: "right" },
  formInput: { backgroundColor: Colors.background, borderWidth: 1, borderColor: Colors.border, borderRadius: 8, padding: 12, fontSize: 14, color: Colors.text, fontFamily: "Vazirmatn" },
  textArea: { minHeight: 60, textAlignVertical: "top" },
  
  methodRow: { flexDirection: "row", gap: 8 },
  methodChip: { flex: 1, paddingVertical: 10, borderRadius: 8, backgroundColor: Colors.background, borderWidth: 1, borderColor: Colors.border, alignItems: "center" },
  methodChipActive: { backgroundColor: Colors.primary, borderColor: Colors.primary },
  methodChipText: { fontSize: 13, color: Colors.textSecondary, fontFamily: "Vazirmatn" },
