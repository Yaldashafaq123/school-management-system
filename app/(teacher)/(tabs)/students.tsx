...
export default function StudentsManagement() {
  ...
  const [sortBy, setSortBy] = useState<"name" | "progress" | "recent">(
    "recent",
  );
  const [refreshing, setRefreshing] = useState(false);
  const [selectedStudents, setSelectedStudents] = useState<number[]>([]);

  // Fetch students from API - NO MOCK DATA
  ...
  // Filter and sort students - NO MOCK DATA FALLBACK
  const filterAndSortStudents = useCallback(() => {
    ...
    // Apply status filter
    if (filterBy === "active") {
      filtered = filtered.filter((student) => (student.progress || 0) > 50);
  ...
  const handleBulkAction = async (action: "message" | "remove" | "export") => {
    ...
    switch (action) {
      case "message":
            ...
            { text: "لغو", style: "cancel" },
            {
              ...
              onPress: async (message: string | undefined) => {
                if (message) {
                  try {
                    const token = await AsyncStorage.getItem("auth_token");
                    // Send to first student as example (bulk messaging would need backend support)
                    const studentId = selectedStudents[0];
                    await fetch(
                      `${BASE_URL}/teacher/students/${studentId}/message`,
                      {
                        method: "POST",
                        headers: {
        ...
        break;
      case "remove":
        Alert.alert("حذف دانش‌آموزان", `این عملیات در حال حاضر غیرفعال است.`, [
  ...
  const renderStudentCard = (student: Student) => (
    ...
    >
      ...
      <Image
        source={{
          uri: student.profile_image || "https://via.placeholder.com/50",
        }}
      ...
      <View style={styles.studentInfo}>
        ...
        <View style={styles.studentMeta}>
          ...
          <View style={styles.metaItem}>
            ...
            />
            <Text style={styles.metaText}>
              {student.assignments_completed || 0} کارخانگی انجام شده
            </Text>
          </View>
          <View style={styles.metaItem}>
            <Ionicons name="time" size={12} color={Colors.textSecondary} />
            <Text style={styles.metaText}>
              {student.last_active || "نامشخص"}
            </Text>
          </View>
        </View>
      </View>
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <View style={styles.filtersContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={styles.filterButtons}>
            ...
            >
              ...
              >
                فعال ({students.filter((s) => (s.progress || 0) > 50).length})
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[
                styles.filterButton,
                filterBy === "inactive" && styles.filterButtonActive,
              ]}
            ...
            >
              ...
              <Text
                style={[
                  styles.filterButtonText,
                  filterBy === "inactive" && styles.filterButtonTextActive,
                ]}
              >
                نیاز به توجه (
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            Alert.alert("مرتب‌سازی", "بر اساس چه معیاری مرتب شود؟", [
      ...
      >
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateText}>
              {searchQuery
                ...
                : "هنوز دانش‌آموزی در صنف شما ثبت‌نام نکرده است."}
            </Text>
            {!searchQuery && (
              <TouchableOpacity
        ...
        {/* Stats Overview - Only show if there are students */}
        {students.length > 0 && (
          <View style={styles.statsOverview}>
            ...
            <View style={styles.statsGrid}>
              ...
              <View style={styles.statItem}>
                <Text style={styles.statValue}>{stats.average_progress}%</Text>
              ...
              <View style={styles.statItem}>
                <Text style={styles.statValue}>{stats.total_assignments}</Text>
                <Text style={styles.statLabel}>کارخانگی انجام شده</Text>
              </View>
              <View style={styles.statItem}>
