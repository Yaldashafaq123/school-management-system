...
export default function Grading() {
  ...
  const fetchAssignments = useCallback(async () => {
    try {
      console.log("Fetching assignments for grading...");
      
      setLoading(true);

      // First get all assignments with their submissions
      const assignmentsResponse = await apiRequest('/teacher/assignments');
      
      console.log('Assignments response:', assignmentsResponse);
      
      const pendingList: Assignment[] = [];
      const gradedList: Assignment[] = [];
      const allSubs: Submission[] = [];

      if (assignmentsResponse?.success && assignmentsResponse.data) {
        // Loop through each assignment
        for (const assignment of assignmentsResponse.data) {
          // Get detailed assignment info with submissions
          const detailResponse = await apiRequest(`/teacher/assignments/${assignment.id}/grading`);
          
          if (detailResponse?.success && detailResponse.data) {
            const assignmentData = detailResponse.data;
            
            // Format submissions
            const submissions = (assignmentData.submissions || []).map((sub: any) => ({
              id: sub.id,
              ...
              submitted_at: new Date(sub.submitted_at).toLocaleDateString('fa-IR'),
              status: sub.status,
              grade: sub.grade,
              max_grade: assignmentData.max_score,
              feedback: sub.feedback,
              graded_at: sub.graded_at ? new Date(sub.graded_at).toLocaleDateString('fa-IR') : undefined
            }));

            allSubs.push(...submissions);

            // Separate into pending and graded
            submissions.forEach((sub: any) => {
              const formattedSub: Assignment = {
                id: sub.id,
                assignmentId: assignmentData.id,
                title: assignmentData.title,
                course: assignmentData.course_name,
              ...
              if (sub.status === 'GRADED') {
                gradedList.push(formattedSub);
              } else {
                pendingList.push(formattedSub);
              }
            });
          }
        }
        ...
        setGradedAssignments(gradedList);
        ...
        const gradedGrades = gradedList
          .filter(g => g.grade !== null)
          .map(g => g.grade as number);
        
        const avgGrade = gradedGrades.length > 0 
          ? gradedGrades.reduce((a, b) => a + b, 0) / gradedGrades.length 
          : 0;

        setStats({
          total_pending: totalPending,
          total_graded: totalGraded,
          average_grade: parseFloat(avgGrade.toFixed(1))
        });
      }
  ...
  const handleGradeAssignment = (assignmentId: number) => {
    ...
    if (assignment) {
      router.push({
        pathname: `/(teacher)/assignment/${assignment.assignmentId}/grading` as any,
        params: { 
  ...
  const handleSelectAll = () => {
    const assignments = activeFilter === 'pending' ? filteredPending : filteredGraded;
  ...
  const handleBulkGrade = () => {
    ...
    Alert.alert('در حال توسعه', 'تصحیح گروهی به زودی اضافه می‌شود');
  };
  ...
  const filteredAll = [...filteredPending, ...filteredGraded];
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View style={styles.statsGrid}>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>
              {stats.total_graded + stats.total_pending > 0
                ? Math.round((stats.total_graded / (stats.total_graded + stats.total_pending)) * 100)
                : 0
              }%
            </Text>
      ...
      >
        ...
        ) : activeFilter === 'all' && filteredAll.length === 0 ? (
          <View style={styles.emptyState}>
            <Ionicons name="document-text" size={60} color={Colors.textSecondary} />
            <Text style={styles.emptyStateTitle}>هیچ کارخانگی یافت نشد</Text>
            <Text style={styles.emptyStateText}>
              هنوز کارخانگی برای نمایش وجود ندارد.
            </Text>
          </View>
        ) : (
              ...
              <Text style={styles.listCount}>
                {activeFilter === 'pending' 
                  ...
                  : activeFilter === 'graded'
                    ...
                    : filteredAll.length} کارخانگی
              </Text>
            </View>

            {activeFilter === 'pending' ? (
              filteredPending.map(assignment => renderAssignmentCard(assignment, true))
            ) : activeFilter === 'graded' ? (
              filteredGraded.map(assignment => renderAssignmentCard(assignment, false))
            ) : (
              filteredAll.map(assignment => 
                renderAssignmentCard(assignment, assignment.status === 'pending')
              )
            )}
          </View>
        )}
        ...
        <View style={styles.quickActionsContainer}>
          ...
          <View style={styles.quickActionsGrid}>
            <TouchableOpacity
              ...
              onPress={() => router.push('/(teacher)/assignment' as any)}
            >
